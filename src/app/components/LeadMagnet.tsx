import React, { useState } from "react";
import { Link } from "react-router-dom";
import { toast } from "sonner";
import { leadMagnetApi } from "@/features/lead-magnet/api";
import { Container, Section } from "./Layout";
import { CheckIcon, FreeResourceIcon } from "./Icons";
import { useTranslation } from "react-i18next";
import { trackEvent } from "@/utils/analytics";

const FeaturePoint = ({ text }: { text: string }) => (
  <div className="flex items-start gap-3">
    <div className="w-5 h-5 bg-black rounded-full flex items-center justify-center shrink-0 mt-0.5">
      <CheckIcon />
    </div>
    <span className="font-['Montserrat'] font-bold text-[14px] leading-[21px] text-black">{text}</span>
  </div>
);

export const LeadMagnet = () => {
  const { t } = useTranslation();
  const [email, setEmail] = useState("");
  const [isSubmitting, setIsSubmitting] = useState(false);
  
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setIsSubmitting(true);
    
    trackEvent("lead_form_submitted", {
      source: "lead_magnet",
      email_domain: email.split("@")[1],
    });
    
    try {
      const { error } = await leadMagnetApi.createSignup({
        email,
        source: "lead_magnet",
      });
      if (error) {
        throw error;
      }
      setEmail("");
      toast.success(t("leadMagnet.success", "You're in. Check your email for the cheat sheet."));
    } catch (err) {
      trackEvent("lead_form_error", { error: String(err) });
      toast.error(t("leadMagnet.error", "Something went wrong. Please try again."));
    } finally {
      setIsSubmitting(false);
    }
  };

  const features = [
    t("leadMagnet.feature1", "Handle small talk with confidence"),
    t("leadMagnet.feature2", 'Understand casual "street" Czech'),
    t("leadMagnet.feature3", "Avoid awkward silences"),
  ];

  return (
    <Section className="bg-[var(--ds-color-accent-base)] border-t border-b border-[var(--ds-color-neutral-900)]">
      <Container>
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 md:gap-16 lg:gap-24 items-start">
          <div className="space-y-6 md:space-y-12">
            <div className="inline-flex items-center gap-3 bg-black/8 border border-[var(--ds-color-neutral-900-20)] rounded-full px-3 md:px-4 py-1.5 md:py-2">
              <FreeResourceIcon />
              <span className="type-ui-sm text-[var(--ds-color-neutral-900)]">
                {t("leadMagnet.badge", "Free Resource")}
              </span>
            </div>
            
            <h2 className="font-[var(--ds-font-family-display)] font-[var(--ds-font-weight-bold)] text-[var(--ds-type-h2-size-sm)] sm:text-[var(--ds-type-h2-size-md)] md:text-[var(--ds-type-h0-size-sm)] leading-[var(--ds-type-h1-line-height)] tracking-[var(--ds-type-h1-letter-spacing)] text-[var(--ds-color-neutral-900)]">
              {t("leadMagnet.title1", "STOP")}
              <br />
              {t("leadMagnet.title2", "SOUNDING")}
              <br />
              {t("leadMagnet.title3", "LIKE A TEXTBOOK.")}
            </h2>
            
            <div className="space-y-4 md:space-y-6">
              <p className="font-[var(--ds-font-family-display)] font-[var(--ds-font-weight-medium)] text-[16px] md:text-[18px] leading-[24px] md:leading-[28px] text-[var(--ds-color-neutral-900-80)] max-w-md">
                {t(
                  "leadMagnet.description1",
                  "Most courses teach you grammar tables. Locals don't speak in tables.",
                )}
              </p>
              <p className="font-[var(--ds-font-family-display)] font-[var(--ds-font-weight-medium)] text-[16px] md:text-[18px] leading-[24px] md:leading-[28px] text-[var(--ds-color-neutral-900-80)] max-w-md">
                {t("leadMagnet.description2", "Download our ")}
                <span className="font-bold">{t("leadMagnet.sheetName", '"Natural Czech Cheat Sheet"')}</span>
                {t(
                  "leadMagnet.description3",
                  ": 50 essential phrases to soften requests, fill pauses, and sound like you actually live here.",
                )}
              </p>
            </div>
            
            <div className="flex flex-col gap-4">
              {features.map((feature) => (
                <FeaturePoint key={feature} text={feature} />
              ))}
            </div>
          </div>
          
          <div className="bg-[var(--ds-color-neutral-0)] border border-[var(--ds-color-neutral-900)] p-6 md:p-12 shadow-[var(--ds-shadow-dense-lg)] md:shadow-[var(--ds-shadow-dense-xl)] w-full lg:sticky lg:top-32">
            <form className="flex flex-col gap-4 md:gap-6" onSubmit={handleSubmit}>
              <div className="space-y-2">
                <label className="type-ui-sm text-[var(--ds-color-neutral-900)] block">
                  {t("leadMagnet.emailLabel", "Work Email")}
                </label>
                <input 
                  type="email" 
                  placeholder={t("leadMagnet.emailPlaceholder", "name@company.com")}
                  value={email}
                  onChange={(e) => setEmail(e.target.value)}
                  required
                  className="w-full h-[48px] md:h-[52px] bg-[var(--ds-color-neutral-0)] border border-[var(--ds-color-neutral-900)] px-3 md:px-4 text-[14px] font-[var(--ds-font-family-display)] text-[var(--ds-color-neutral-900)] focus:outline-none focus:ring-2 focus:ring-[var(--ds-color-neutral-900)] focus:ring-offset-2 transition-all duration-200"
                />
              </div>
              <button 
                type="submit"
                disabled={isSubmitting}
                className="w-full h-[48px] md:h-[52px] bg-[var(--ds-color-neutral-900)] text-[var(--ds-color-neutral-0)] type-ui-md hover:bg-[var(--ds-color-neutral-800)] focus:outline-none focus:ring-2 focus:ring-[var(--ds-color-neutral-900)] focus:ring-offset-2 transition-all duration-200 disabled:opacity-50"
              >
                {isSubmitting ? t("leadMagnet.submitting", "Loading...") : t("leadMagnet.cta", "Get the Cheat Sheet")}
              </button>
              <p className="text-center text-[12px] text-[var(--ds-color-neutral-500)] font-[var(--ds-font-family-display)] leading-[18px]">
                {t("leadMagnet.privacy", "We respect your inbox. Unsubscribe at any time.")}
              </p>
            </form>

            <div className="border-t border-[var(--ds-color-neutral-900-10)] mt-6 md:mt-8 pt-6 md:pt-8 flex flex-col gap-4">
              <div className="space-y-2">
                <h3 className="font-[var(--ds-font-family-display)] font-[var(--ds-font-weight-bold)] text-[18px] md:text-[20px] text-[var(--ds-color-neutral-900)]">
                  {t("leadMagnet.cheatSheetTitle", "Prefer instant access?")}
                </h3>
                <p className="font-[var(--ds-font-family-body)] text-[13px] md:text-[14px] text-[var(--ds-color-neutral-900-70)]">
                  {t(
                    "leadMagnet.cheatSheetBody",
                    "Open the free cheat sheet now and start using the phrases today.",
                  )}
                </p>
              </div>
              
              <Link 
                to="/cheat-sheet"
                className="w-full h-[48px] md:h-[52px] bg-[var(--ds-color-neutral-0)] text-[var(--ds-color-neutral-900)] border border-[var(--ds-color-neutral-900)] type-ui-md hover:translate-x-1 hover:translate-y-1 hover:shadow-none transition-all duration-200 shadow-[var(--ds-shadow-dense-md)] md:shadow-[var(--ds-shadow-dense-lg)] flex items-center justify-center"
              >
                {t("leadMagnet.cheatSheetCta", "View Cheat Sheet")}
              </Link>
              
              <p className="text-center text-[12px] text-[var(--ds-color-neutral-500)] font-[var(--ds-font-family-display)] leading-[18px]">
                {t("leadMagnet.cheatSheetNote", "No email required for the preview.")}
              </p>
            </div>
          </div>
        </div>
      </Container>
    </Section>
  );
};
